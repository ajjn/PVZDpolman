...
class Test03_basic_happy_cycle(unittest.TestCase):
    def runTest(self):
        ...
        requ1_result = os.path.abspath('%s//redmineIdentineticsCom_idp.xml' % pepoutdir)
        os.path.isfile(requ1_result)
        assert os.path.isfile(os.path.abspath(requ1_result)), 'expected %s in pepoutdir directory' % requ1_result
